...
// English course lesson list (Phase 2C.2). Hydrates from server-side progress
// when available; otherwise falls back to the local mirror (and flags it). Also
// offers a one-time sync of leftover local progress to the server.
...
export default function EnglishLessonList({
  serverOk,
  learnedPhraseIds,
  ...
  useEffect(() => {
    const localLearned = getEnLearned();
    const localVoice = getEnVoicePassed();
    if (serverOk) {
      setLearned(new Set(learnedPhraseIds));
      setCompleted(new Set(completedLessonIds));
      // Detect local progress that isn't on the server yet (offer one-time sync).
      ...
      // Fallback: local mirror is the only source.
      const ls = new Set(localLearned);
      setLearned(ls);
      const comp = new Set<string>();
      for (const m of ENGLISH_COURSE.modules) {
  ...
  async function doSync() {
    setSyncing(true);
    setSyncMsg(null);
    const res = await syncLocalEnglishProgress({
      learned: localLeftover.learned,
      voicePassed: localLeftover.voice,
    });
    setSyncing(false);
    if (res.ok) {
    ...
    <>
      <section className="rounded-2xl bg-white p-4 shadow-card ring-1 ring-gray-100">
        <div className="flex items-center justify-between">
          <span className="text-sm font-semibold text-ink">Tiến độ tiếng Anh</span>
      ...
      {hasLeftover && (
        <section className="rounded-2xl bg-brand-50 p-3 ring-1 ring-brand-100">
          <p className="text-xs text-brand-800">
            Phát hiện tiến độ tiếng Anh cũ trên thiết bị này ({localLeftover.learned.length} câu) chưa có trên máy chủ.
          </p>
          <button
            onClick={doSync}
            disabled={syncing}
      ...
      <p className="rounded-xl bg-amber-50 p-3 text-xs text-amber-800 ring-1 ring-amber-100">
        Nội dung tiếng Anh đang chờ duyệt nội bộ. Tiến độ tiếng Anh được lưu trên máy chủ và đồng bộ giữa các thiết bị
        khi bạn đăng nhập.
      </p>
    </>
  );
