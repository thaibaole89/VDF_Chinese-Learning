#!/usr/bin/env node
// Generates supabase/migrations/002_phase_2a_rpcs.sql from /content/*.json.
//
// The migration is idempotent (safe to re-paste in Supabase SQL Editor).
// Re-run this script whenever content phrases change:
//   node scripts/gen-phase2a-rpcs.mjs
//
// The OUTPUT file is committed (so reviewers can read it without running node);
// THIS script is the source of truth for the phrase-seed block.
...
const OUT = join(ROOT, "supabase/migrations/002_phase_2a_rpcs.sql");
...
for (const file of files) {
  ...
  for (const lesson of data.lessons ?? []) {
    for (const sp of lesson.sentencePatterns ?? []) {
      if (sp.id && sp.zh) phrases.push({ id: sp.id, zh: sp.zh, lesson_id: lesson.id });
    }
  }
}
phrases.sort((a, b) => a.id.localeCompare(b.id));
console.log(`Collected ${phrases.length} sentence patterns from ${files.length} content files`);

// ----- SQL escape -----
const sql = (s) => "'" + String(s).replace(/'/g, "''") + "'";

const seedValues = phrases
  .map((p) => `  (${sql(p.id)}, ${sql(p.zh)}, ${sql(p.lesson_id)})`)
  .join(",\n");

// ----- assemble migration -----
// IMPORTANT: keep this file self-contained, idempotent, and human-readable.
// All CREATE statements use IF NOT EXISTS or CREATE OR REPLACE; all policies
// drop+recreate; all triggers drop+recreate.
const out = `-- Phase 2A.2a — server-side voice scoring + lockdown (IDEMPOTENT).
...
--
-- Phrases seeded: ${phrases.length} from ${files.length} content files.
...
delete from public.phrases
 where id not in (${phrases.map((p) => sql(p.id)).join(", ")});
...
-- Expected: phrases_seeded=${phrases.length}, functions_created=9, pp_policy_count=4, voice_insert_grants=0.
